...
const reviewTemplates = {
  '백숙삼계탕': [
    ...
    '닭통살이 부드럽고糯차가 찐득해서 같이 먹으면 환상이에요.',
...
function getReviewType(name) {
  ...
  if (lowerName.includes('해물탕') || lowerName.includes('해물') || lowerName.includes('해장국')) return '해물탕';
...
function getCategory(name) {
  ...
  if (lowerName.includes('양식') || lowerName.includes('피자') || lowerName.includes('파스타')) return 'western';
  if (lowerName.includes('분식') || lowerName.includes('떡볶이')) return 'bunsik';
  if (lowerName.includes('카페') || lowerName.includes('베이커리')) return 'cafe';
  return 'korean';
}
...
console.log(`Generated ${count} place files with reviews and recommenders`);
